test_that("ps_get_season", {
  ...
  expect_identical(
    ps_get_season(dates, seasons = c(Monsoon = 2L, `Dry Period` = 6L)),
    ordered(
      c(
        "Dry Period",
        "Monsoon",
...
test_that("ps_add_season", {
  x <- data.frame(
    Date = as.Date(c("2000-12-31", "2001-01-01", "2001-06-01", "2001-12-31"))
  )
  y <- ps_add_season(x)
  ...
  z <- ps_add_season(
    x,
    year_season = "Yr",
    seasons = c(Spring = 1L, Summer = 6L, Winter = 12L)
  )
  expect_identical(colnames(z), c("Date", "Season", "Yr"))
  expect_identical(
    z$Season,
    ordered(
      c("Winter", "Spring", "Summer", "Winter"),
